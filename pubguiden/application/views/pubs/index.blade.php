...
<div class="content container">
	...
	<div class="pub-head"><div class="big-rating"> {{ $ratings }} </div> <h1> {{ $pub->name }} </h1></div>
		<div class="pub-text"> <i class="icon-bookmark"></i>{{ $pub->description }} </div>
	</section>

	<section class="pub-info group-holder">
		<div class="pub-text"> <i class="icon-home"></i> <strong>Adress:</strong> {{ $pub->address }}</div>		
		<div class="pub-text"> <i class="icon-money"></i> <strong>Billigaste öl:</strong> {{ $pub->lowest_price }} kr</div>
		<div class="pub-text"> <i class="icon-trophy"></i> <strong>Quiz?</strong> 
			@if($pub->quiz == 0)
		    	Nej
		    @else
		    	Ja
		...
		<div class="pub-text"> <i class="icon-food"></i> <strong> After Work? </strong>
			@if($pub->after_work == 0)
		    	Nej
		    @else
		    	Ja
