...
<section class="content container ratings">
	<section class="group-holder">
		@foreach($pubs as $pub)
			<h1>Betygsätt {{$pub->name}}</h1>
		@endforeach
		<p class="info-block"> Välj antal stjärnor du tycker puben förtjänar per kategori, tryck på skicka när du är klar. Du kan uppdatera ditt betyg senare om du skulle ändra dig!</p>
		{{ Form::open() }}
